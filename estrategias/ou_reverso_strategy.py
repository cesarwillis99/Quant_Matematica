...
"""
================================================
ou_reverso_strategy.py — Estratégia Quantitativa (Breakout)
...
Uso:
  ...
  3. Executar: python ou_reverso_strategy.py
================================================
"""
...
def gerar_grafico_ou(df: pd.DataFrame):
    ...
    fig.suptitle(f"Ornstein-Uhlenbeck Reverso (OU Breakout) — {ATIVO} {TIMEFRAME}", fontsize=16, fontweight='bold', color='#FFFFFF')
    ...
    # Linhas de threshold
    ax2.axhline(2.0,  color=cor_compra, linestyle='--', alpha=0.8, linewidth=1.0, label='Entrada Compra Breakout (+2.0)')
    ax2.axhline(-2.0, color=cor_venda,  linestyle='--', alpha=0.8, linewidth=1.0, label='Entrada Venda Breakout (-2.0)')
    ...
    # Breakout: verde quando zscore alto (LONG), vermelho quando zscore baixo (SHORT)
...
def processar_pipeline_ou(forcar: bool = False) -> pd.DataFrame:
    ...
    if not PARQUET_COMPLETO.exists():
        raise FileNotFoundError(
            f"Parquet de entrada completo não encontrado em: {PARQUET_COMPLETO}\n"
...
def carregar_ou_reverso() -> pd.DataFrame:
    """
    Carrega o parquet com os indicadores OU calculados.
    """
    if not PARQUET_SAIDA.exists():
        raise FileNotFoundError(
            f"Parquet com indicadores OU Reverso não encontrado em: {PARQUET_SAIDA}\n"
            f"Por favor, execute o módulo para gerá-lo: python ou_reverso_strategy.py"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=f"Módulo Ornstein-Uhlenbeck Reverso (Breakout) — {ATIVO} {TIMEFRAME}",
    ...
    print("\n" + "█" * 70)
    print("█" + " " * 68 + "█")
    print(f"█   ESTRATÉGIA OU REVERSO (BREAKOUT) {ATIVO} {TIMEFRAME}                 █")
    print("█   Operação: Breakout — A FAVOR do desvio extremo              █")
    print("█   Inverso lógico do ou_strategy.py                            █")
    print("█" + " " * 68 + "█")
    print("█" * 70)
    
    try:
        processar_pipeline_ou(args.forcar)
        print("✅ Módulo ou_reverso_strategy.py executado com sucesso!\n")
    except Exception as e:
        logger.exception("Erro crítico durante a execução do módulo ou_reverso_strategy.py:")
        sys.exit(1)
