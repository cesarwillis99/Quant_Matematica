# -*- coding: utf-8 -*-
"""
Otimização para o GRID 4 — Gatilho baseado no rompimento das bandas da TWAP.
"""
...
PARAMS_ESPECIFICOS = {
    'janela_twap':  [12, 24, 48, 72],
    'mult_banda':   [1.5, 2.0, 2.5, 3.0],
}

def rodar_otimizacao_grid4():
    """
    Executa a otimização de parâmetros por Grid Search para o Grid 4.
    """
    ...
    logger.info("Carregando base de dados para otimização do Grid 4...")
    ...
    logger.info(f"Iniciando Grid Search do Grid 4: {total_combinacoes} combinações totais.")
    ...
    for i_esp, p_esp in enumerate(combinacoes_especificas):
        ...
        df_sinalizado = gerar_sinal_twap_band(
            df,
            janela=p_esp['janela_twap'],
            mult_banda=p_esp['mult_banda']
        )
        
        # Se não houver nenhum sinal disparado, pula a execução dos parâmetros comuns
        if (df_sinalizado['sinal_grid4'] != 0).sum() == 0:
            continue
        ...
        for p_com in combinacoes_comuns:
            ...
            try:
                # Rodar Backtest
                res = rodar_backtest_grid(df_sinalizado, tipo_grid=4, params=params_teste, capital_inicial=CAPITAL_INICIAL, verbose=False)
                
                # Filtro mínimo exigido: total_grids >= 30 e profit_factor > 1.0
                if res['total_grids'] >= 30 and res['profit_factor'] > 1.0:
                    resultados.append({
                        'janela_twap': params_teste['janela_twap'],
                        'mult_banda': params_teste['mult_banda'],
    ...
    caminho_parquet = DIR_OTIM / "otimizacao_grid4_completo.parquet"
    ...
    for idx, row in top10_df.iterrows():
        top10_lista.append({
            "id": f"GRID4_TOP{idx + 1}",
            "tipo_grid": 4,
            "janela_twap": int(row['janela_twap']),
            "mult_banda": float(row['mult_banda']),
    ...
    # Salvar JSON Top 10
    caminho_json = DIR_OTIM / "otimizacao_grid4_top10.json"
    ...
    print("\n" + "═"*125)
    print(" " * 48 + "TABELA TOP 10 OTIMIZAÇÃO GRID 4")
    print("═"*125)
    print(f"{'Rank':<5}{'Jan. TWAP':<11}{'Mult Banda':<12}{'Mult Esp.':<11}{'Mult Alvo':<11}{'Mult Stop':<11}{'Stop Cand.':<12}{'N Grids':<9}{'WinRate':<9}{'ProfitF.':<10}{'FatRecup.':<11}")
    print("─"*125)
    for idx, item in enumerate(top10_lista):
        print(f"#{idx+1:<4}{item['janela_twap']:<11}{item['mult_banda']:<12.2f}{item['mult_espacamento']:<11.1f}{item['mult_alvo']:<11.1f}{item['mult_stop']:<11.1f}{item['stop_candles']:<12}{item['total_grids']:<9}{item['win_rate']:<9.1f}%{item['profit_factor']:<10.2f}{item['fator_recuperacao']:<11.2f}")
    print("═"*125 + "\n")

if __name__ == "__main__":
    rodar_otimizacao_grid4()
