...
"""
================================================================================
data_loader.py — Módulo de Carregamento e Pré-Processamento de Dados USDCHF
                  VERSÃO 2 — DEFINITIVA
================================================================================
...
Saídas:
    1. usdchf_h1_completo.parquet     — Série completa seg-sex 00h05-23h55
       Uso: cálculo de indicadores + monitoramento de stops/targets incluindo madrugada
    2. usdchf_h1_operacional.parquet  — Apenas 10h00-22h30, seg-sex
...
CSV_PADRAO = DIR_PROJETO.parent / "USDCHF_10ANOS.csv"

# Arquivos de saída
PARQUET_COMPLETO     = DIR_DATA / "usdchf_h1_completo.parquet"
PARQUET_OPERACIONAL  = DIR_DATA / "usdchf_h1_operacional.parquet"
...
# =============================================================================
# FILTRO DE HORÁRIOS USDCHF (SÉRIE COMPLETA E OPERACIONAL)
# =============================================================================
def filtro_serie_completa(df: pd.DataFrame) -> pd.DataFrame:
    """
    Filtra a série completa para o horário real de
    funcionamento do USDCHF no servidor MT5.

    O servidor MT5 está em UTC+2/UTC+3 (europeu).
    O USDCHF opera de segunda a sexta: 00h05 até 23h55.
...
def imprimir_relatorio(
    ...
    # ═══════════════════════════════════════════════════════════════════
    # SEÇÃO 2 — Série completa (usdchf_h1_completo.parquet)
    # ═══════════════════════════════════════════════════════════════════
    print(f"\n{sep}")
    print("  SEÇÃO 2 — SÉRIE COMPLETA (usdchf_h1_completo.parquet)")
    print(sep)
    ...
    # ═══════════════════════════════════════════════════════════════════
    # SEÇÃO 3 — Série operacional (usdchf_h1_operacional.parquet)
    # ═══════════════════════════════════════════════════════════════════
    print(f"\n{sep}")
    print("  SEÇÃO 3 — SÉRIE OPERACIONAL (usdchf_h1_operacional.parquet)")
    print(sep)
...
def carregar_e_processar(
    ...
    """
    Executa o pipeline completo de processamento de dados USDCHF M1 → H1.
    ...
    print("  USDCHF M1 → H1 (Série Completa seg-sex 00h05-23h55)")
    print("═" * 70)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Data Loader v2 — USDCHF M1 → H1 (Série Completa 24h FOREX)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemplos de uso:
  python data_loader.py --csv ../USDCHF_10ANOS.csv
  python data_loader.py --csv ../USDCHF_10ANOS.csv --forcar
  python data_loader.py  (usa cache se existir)
    ...
    print("\n" + "█" * 70)
    print("█" + " " * 68 + "█")
    print("█   DATA LOADER v2 — USDCHF M1 → H1                               █")
