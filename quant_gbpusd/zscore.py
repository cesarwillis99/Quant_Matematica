...
PARQUET_SAIDA   = DIR_DATA / "gbpusd_h1_zscore.parquet"
...
def calcular_indicadores_zscore(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["sl_pips"] = (3.0 * df["vr_pips"]).astype("float32")
    df["tp_pips"] = (2.0 * df["vr_pips"]).astype("float32")
...
def gerar_sinais_zscore(df: pd.DataFrame) -> tuple:
    ...
    # Gatilho Direto
    cond_long = condicao_entrada & (z <= -z_entry)
    cond_short = condicao_entrada & (z >= z_entry)
    
    cond_long = cond_long & (~df["zscore"].isna())
    cond_short = cond_short & (~df["zscore"].isna())
    ...
    potencial_sem_regime = op_window & ((z <= -z_entry) | (z >= z_entry))
...
def gerar_grafico_zscore_sinais(df: pd.DataFrame):
    ...
    fig.suptitle("Estratégia Mean Reversion por Z-Score — GBPUSD H1", fontsize=16, fontweight='bold', color='#FFFFFF')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Módulo Mean Reversion por Z-Score — GBPUSD H1",
    ...
    print("\n" + "█" * 70)
    print("█" + " " * 68 + "█")
    print("█   ESTRATÉGIA MEAN REVERSION POR Z-SCORE GBPUSD v2             █")
