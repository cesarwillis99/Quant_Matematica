...
Objetivo:
    ...
    de Reversão à Média para o GBPUSD H1. Classificar o mercado em regimes de:
...
Saída:
    - data/gbpusd_h1_hurst.parquet (Série completa com as colunas hurst, regime e half_life)
...
PARQUET_SAIDA   = DIR_DATA / "gbpusd_h1_hurst.parquet"
...
def calcular_hurst_rolling(df: pd.DataFrame, janela: int = 100) -> pd.DataFrame:
    ...
    Parâmetros:
        df: DataFrame contendo a série de preços GBPUSD H1 completa
...
def gerar_grafico_regime(df: pd.DataFrame):
    ...
    GBPUSD com coloração por regime e a evolução do Hurst.
    ...
    fig.suptitle("GBPUSD H1 — Árbitro de Regime por Expoente de Hurst", fontsize=16, fontweight='bold', color='#FFFFFF')
    ...
    # -------------------------------------------------------------------------
    # PAINEL 1 — Preço Close GBPUSD com coloração por Regime
    # -------------------------------------------------------------------------
    ax1.plot(df_plot.index, df_plot["Close"], color='#ECEFF1', linewidth=1.2, label='Preço GBPUSD')
    ax1.set_title("Identificação de Regime sobre a Série de Preços", fontsize=12, fontweight='semibold', color='#CFD8DC')
    ax1.set_ylabel("GBPUSD H1 Close", fontsize=11, color='#CFD8DC')
...
def processar_pipeline_hurst(forcar: bool = False) -> pd.DataFrame:
    ...
    logger.info("Reprocessando base completa do GBPUSD H1 para o cálculo de Hurst...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Árbitro de Regime GBPUSD H1 — Cálculo do Expoente de Hurst e Meia-Vida",
    ...
    print("\n" + "█" * 70)
    print("█" + " " * 68 + "█")
    print("█   ÁRBITRO DE REGIME GBPUSD v2 — EXPOENTE DE HURST              █")
