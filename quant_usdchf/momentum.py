...
Saída:
    - data/usdchf_h1_momentum.parquet
...
PARQUET_ENTRADA = DIR_DATA / "usdchf_h1_hurst.parquet"
PARQUET_SAIDA   = DIR_DATA / "usdchf_h1_momentum.parquet"
...
def gerar_sinais_momentum(df: pd.DataFrame) -> tuple:
    ...
    # Usamos o Hurst > 0.60 diretamente do parquet
    c1_regime = (df["hurst"] > 0.60)
    c2_entropia_operavel = (df["entropia_shannon"] < 0.65)
...
def gerar_grafico_momentum_sinais(df: pd.DataFrame):
    ...
    Painel 1: Preço USDCHF com setas de compra e venda
    ...
    fig.suptitle("Estratégia Momentum & Entropia — USDCHF H1", fontsize=16, fontweight='bold', color='#FFFFFF')
    ...
    ax1.set_ylabel("Preço USDCHF", fontsize=11, color='#CFD8DC')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Módulo Momentum e Entropia — USDCHF H1",
    ...
    print("\n" + "█" * 70)
    print("█" + " " * 68 + "█")
    print("█   ESTRATÉGIA DE MOMENTUM E ENTROPIA USDCHF v2                 █")
