...
"""
================================================================================
oos_backtest_passado_zscore.py — Teste Out-of-Sample (OOS) PASSADO
================================================================================
Script 100% autocontido para validação OOS da estratégia ZSCORE.
...
def recalcular_sinais_oos(df_completo: pd.DataFrame, df_operacional: pd.DataFrame) -> pd.DataFrame:
    """
    Recalcula os sinais da estratégia ZSCORE sobre o período OOS
    usando os MESMOS parâmetros do treinamento.
    """
    df = df_completo.copy()
    
    # 1. Z-Score Rolling
    sma = df["Close"].rolling(100, min_periods=100).mean()
    std = df["Close"].rolling(100, min_periods=100).std(ddof=1)
    df["zscore"] = (df["Close"] - sma) / std
    
    # 2. Gestão de Risco (Volatilidade)
    ...
    # 4. Sinais (Reversão à Média: Z-Score extremado)
    sinal = np.zeros(len(df), dtype=np.int8)
    # Supondo thresholds simétricos padronizados do Z-Score
    cond_long = janela_op & (df["zscore"] < -2.0)
    cond_short = janela_op & (df["zscore"] > 2.0)
