...
Objetivo:
    ...
    de Reversão à Média para o USDCHF H1. Classificar o mercado em regimes de:
...
Saída:
    - data/usdchf_h1_hurst.parquet (Série completa com as colunas hurst, regime e half_life)
...
PARQUET_ENTRADA = DIR_DATA / "usdchf_h1_completo.parquet"
PARQUET_SAIDA   = DIR_DATA / "usdchf_h1_hurst.parquet"
...
def calcular_hurst_rolling(df: pd.DataFrame, janela: int = 100) -> pd.DataFrame:
    ...
    Parâmetros:
        df: DataFrame contendo a série de preços USDCHF H1 completa
...
def gerar_grafico_regime(df: pd.DataFrame):
    ...
    USDCHF com coloração por regime e a evolução do Hurst.
    ...
    fig.suptitle("USDCHF H1 — Árbitro de Regime por Expoente de Hurst", fontsize=16, fontweight='bold', color='#FFFFFF')
    ...
    # -------------------------------------------------------------------------
    # PAINEL 1 — Preço Close USDCHF com coloração por Regime
    # -------------------------------------------------------------------------
    ax1.plot(df_plot.index, df_plot["Close"], color='#ECEFF1', linewidth=1.2, label='Preço USDCHF')
    ax1.set_title("Identificação de Regime sobre a Série de Preços", fontsize=12, fontweight='semibold', color='#CFD8DC')
    ax1.set_ylabel("USDCHF H1 Close", fontsize=11, color='#CFD8DC')
...
def processar_pipeline_hurst(forcar: bool = False) -> pd.DataFrame:
    ...
    logger.info("Reprocessando base completa do USDCHF H1 para o cálculo de Hurst...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Árbitro de Regime USDCHF H1 — Cálculo do Expoente de Hurst e Meia-Vida",
    ...
    print("\n" + "█" * 70)
    print("█" + " " * 68 + "█")
    print("█   ÁRBITRO DE REGIME USDCHF v2 — EXPOENTE DE HURST              █")
