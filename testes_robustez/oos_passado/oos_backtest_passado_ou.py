...
"""
================================================================================
oos_backtest_passado_ou.py — Teste Out-of-Sample (OOS) PASSADO
================================================================================
Script 100% autocontido para validação OOS da estratégia OU.
...
def recalcular_sinais_oos(df_completo: pd.DataFrame, df_operacional: pd.DataFrame) -> pd.DataFrame:
    """
    Recalcula os sinais da estratégia OU sobre o período OOS
    ...
    # 1. Parâmetros OU (Ornstein-Uhlenbeck)
    janela_ou = 100
    residuos = np.full(len(df), np.nan)
    
    # Calculando spread AR(1) OOS (simplificado para performance O(N))
    # Para uma implementação 100% perfeita, o OLS deve rolar:
    precos = df["Close"].values
    for i in range(janela_ou, len(df)):
        y = precos[i-janela_ou+1 : i+1]
        x = precos[i-janela_ou : i]
        # x_sm = sm.add_constant(x)  (pesado)
        # Aproximação rápida de z-score do resíduo (Spread OU)
