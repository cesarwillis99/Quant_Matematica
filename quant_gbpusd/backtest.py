...
"""
================================================================================
backtest.py — Simulação Histórica Multiestratégia GBPUSD H1
================================================================================
...
Mecânica do Backtest:
    - Iteração candle a candle sobre toda a base GBPUSD H1
...
Arquivos de entrada:
    - quant_gbpusd_v2/data/gbpusd_h1_zscore.parquet
    - quant_gbpusd_v2/data/gbpusd_h1_momentum.parquet
    - quant_gbpusd_v2/data/gbpusd_h1_ou.parquet

Arquivos de saída:
    - quant_gbpusd/graficos/equity_curves.png e quant_gbpusd_v2/graficos/equity_curves.png
    - quant_gbpusd/resultados/operacoes.csv e quant_gbpusd_v2/resultados/operacoes.csv
    - quant_gbpusd/resultados/metricas.csv e quant_gbpusd_v2/resultados/metricas.csv
...
DIR_PROJETO_V1 = DIR_ATUAL.parent / "quant_gbpusd"

# Parquets de Entrada (Sempre lidos de V2)
PARQUET_ZSCORE   = DIR_PROJETO_V2 / "data" / "gbpusd_h1_zscore.parquet"
PARQUET_MOMENTUM = DIR_PROJETO_V2 / "data" / "gbpusd_h1_momentum.parquet"
PARQUET_OU       = DIR_PROJETO_V2 / "data" / "gbpusd_h1_ou.parquet"
...
FATOR_PIPS         = 10_000.0   # 1 pip = 0.0001 no GBPUSD
...
def simular_estrategia(
    ...
    sl_o = df["sl_pips_ou"].values.astype(np.float64)
    tp_o = df["tp_pips_ou"].values.astype(np.float64)
...
def gerar_grafico_equity_curves(
    ...
    fig.suptitle(
        "GBPUSD H1 — Equity Curves Históricas e Perfil de Drawdown (ZScore, Momentum, OU, Combinada)",
...
def processar_pipeline_backtest():
    ...
    for p in [PARQUET_ZSCORE, PARQUET_MOMENTUM, PARQUET_OU]:
        if not p.exists():
    ...
    df_o = pd.read_parquet(PARQUET_OU, engine="pyarrow")
    ...
    df["sinal_ou"] = df_o["sinal_ou"].reindex(df.index, fill_value=0)
    df["ou_zscore"] = df_o["ou_zscore"].reindex(df.index)
    df["sl_pips_ou"] = df_o["sl_pips"].reindex(df.index)
    df["tp_pips_ou"] = df_o["tp_pips"].reindex(df.index)
    
    logger.info(f"Dados unificados com sucesso! {len(df):,} candles H1 no período de backtest.")
    
    # 3. Montar sinal combinado de portfólio
    # Prioridade estrita: 1. ZSCORE -> 2. OU -> 3. MOMENTUM
    ...
    logger.info(f"Sinais na base: ZScore={n_z_tot} | OU={n_o_tot} | Momentum={n_m_tot} | Combinada={n_comb_tot}")
    
    # ── Simulações Individuais ──
    ops_z = simular_estrategia(df, "sinal_zscore", "ZSCORE", usar_zscore_exit=True)
    ops_o = simular_estrategia(df, "sinal_ou", "OU", usar_ou_exit=True)
    ...
    equity_curves = {"ZSCORE": eq_z, "OU": eq_o, "MOMENTUM": eq_m, "COMBINADA": eq_c}
    todas_operacoes = {"ZSCORE": ops_z, "OU": ops_o, "MOMENTUM": ops_m, "COMBINADA": ops_c}
    ...
    todas_metricas = {}
    for nome in ["ZSCORE", "OU", "MOMENTUM", "COMBINADA"]:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Backtester Manual Multiestratégia GBPUSD H1",
    ...
    print("\n" + "█" * 70)
    print("█" + " " * 68 + "█")
    print("█   INICIANDO PIPELINE DE BACKTEST MULTIESTRATÉGIA GBPUSD H1   █")
