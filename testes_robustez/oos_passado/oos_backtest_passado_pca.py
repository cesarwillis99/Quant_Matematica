...
"""
================================================================================
oos_backtest_passado_pca.py — Teste Out-of-Sample (OOS) PASSADO
================================================================================
Script 100% autocontido para validação OOS da estratégia PCA.
...
def recalcular_sinais_oos(df_completo: pd.DataFrame, df_operacional: pd.DataFrame) -> pd.DataFrame:
    """
    Recalcula os sinais da estratégia PCA sobre o período OOS
    ...
    # 1. PCA Rolling Approximation (usando z-scores básicos das features como vetor)
    df["feat_ret"] = df["log_return"].rolling(10).sum()
    df["feat_vol"] = df["log_return"].rolling(10).std()
    df["feat_mom"] = df["Close"].diff(10)
    
    # Devido à complexidade computacional do PCA rolante, simplificamos o sinal geométrico OOS
