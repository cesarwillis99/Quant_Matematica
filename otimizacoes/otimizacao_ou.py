...
def calcular_ou_rolling(df: pd.DataFrame, janela: int = 100) -> tuple:
    ...
    for t in tqdm(range(janela - 1, n_rows), desc=f"Processando OU (Janela {janela})"):
        ...
        # Filtro modificado para não excluir halflife_max hardcoded na rolagem 
        # (vai ser filtrado dinamicamente no loop vetorizado)
        if half_life < 1.0:
            continue
...
def main():
    ...
    for janela_ou in janelas_ou:
        print(f"\n[+] Iniciando cálculo da Janela OU = {janela_ou} (Loop Externo)...")
