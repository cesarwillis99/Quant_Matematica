...
"""
================================================================================
zscore.py — Módulo de Reversão à Média por Z-Score do Preço (NASDAQ M10)
================================================================================
...
PARQUET_SAIDA   = DIR_DATA / "nasdaq_m10_zscore.parquet"
...
def gerar_sinais_zscore(df: pd.DataFrame) -> tuple:
    ...
    Filtro de Hurst estrito: hurst < 0.45 (Regime de Reversão ajustado para Nasdaq).
    """
    logger.info("Executando motor de geração de sinais Z-Score otimizado...")
    ...
    # Usamos a coluna hurst < 0.45 (limite de antipersistência para o Nasdaq conforme hurst.py)
    c1_regime = (df["hurst"] < 0.45)
    ...
    # Sinais potenciais (cruzamento de retorno sob Hurst < 0.45)
    ...
    # Bloqueados por regime (cruzamento de retorno dentro do horário, mas com Hurst >= 0.45)
...
def imprimir_relatorio_zscore(df: pd.DataFrame, stats_sinais: dict):
    ...
    print(f"  Relação Retorno:Risco (R:R) Confirmada:  1:{rr_ratio:.2f} (Intencional 1:1.50)")
    print(f"{sep}\n")
...
def gerar_grafico_zscore_sinais(df: pd.DataFrame):
    ...
    fig.suptitle("Estratégia Mean Reversion por Z-Score — NASDAQ M10", fontsize=16, fontweight='bold', color='#FFFFFF')
    ...
    ax1.scatter(compras.index, compras["Close"] - offset, color=cor_compra, marker='^', s=45, label='COMPRA (LONG)', zorder=5)
    ax1.scatter(vendas.index, vendas["Close"] + offset, color=cor_venda, marker='v', s=45, label='VENDA (SHORT)', zorder=5)
    ...
    ax2.axhline(-2.5, color=cor_compra, linestyle='--', alpha=0.7, linewidth=1.0, label='Entrada Compra (-2.5)')
    ax2.axhline(2.5, color=cor_venda, linestyle='--', alpha=0.7, linewidth=1.0, label='Entrada Venda (+2.5)')
    ax2.axhline(-3.0, color=cor_compra, linestyle=':', alpha=0.5, linewidth=0.9, label='Extremo (-3.0)')
    ax2.axhline(3.0, color=cor_venda, linestyle=':', alpha=0.5, linewidth=0.9, label='Extremo (+3.0)')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Módulo Mean Reversion por Z-Score — NASDAQ M10",
    ...
    print("\n" + "█" * 70)
    print("█" + " " * 68 + "█")
    print("█   ESTRATÉGIA MEAN REVERSION POR Z-SCORE NASDAQ M10              █")
    print("█   Filtros: Hurst Reversão + Z-Score Cruzamento (2.5 ou -2.5)    █")
