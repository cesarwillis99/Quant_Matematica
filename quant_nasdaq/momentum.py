...
"""
================================================================================
momentum.py — Módulo de Estratégia de Momentum (Cinemática & Entropia de Shannon)
================================================================================

Objetivo:
    Implementar a estratégia quantitativa de Trend Following baseada em:
    1. Cinemática do Preço (Velocidade, Aceleração e Percentil de Aceleração)
    2. Entropia de Shannon (Medida de desordem dos log-retornos)
    3. Gestão de Risco baseada em Volatilidade Realizada (SL/TP dinâmicos)

Regras de Operação:
    - Opera APENAS quando o Hurst classifica o regime como "TENDENCIA".
    - Indicadores calculados sobre a série COMPLETA (todos os candles).
    - Geração de sinais de entrada restrita à janela OPERACIONAL (16h30-22h30 seg-sex).
    - Gestão de risco monitorada 24h na série completa.

Saída:
    - data/nasdaq_m10_momentum.parquet
    - graficos/momentum_sinais.png (Gráfico de 4 painéis em Dark Mode)
...
PARQUET_ENTRADA = DIR_DATA / "nasdaq_m10_hurst.parquet"
PARQUET_SAIDA   = DIR_DATA / "nasdaq_m10_momentum.parquet"
CAMINHO_GRAFICO = DIR_GRAFICOS / "momentum_sinais.png"
...
def verificar_janela_operacional(dt_index: pd.DatetimeIndex) -> pd.Series:
    """
    Filtra os timestamps que estão dentro do horário operacional de segunda a sexta (16h30 às 22h30).
    """
    weekday = dt_index.weekday
    hora    = dt_index.strftime('%H:%M')

    seg_sex      = (weekday >= 0) & (weekday <= 4)
    horario_op   = (hora >= "16:30") & (hora <= "22:30")
...
def calcular_indicadores_momentum(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Para índices (Nasdaq), 1 ponto = 1.0. Não multiplicamos por 10000 como no Forex.
    df["vr_pips"] = (vr * df["Close"] * 1.0).astype("float32") 
...
def gerar_sinais_momentum(df: pd.DataFrame) -> tuple:
    ...
    # 1. Filtro de Janela Operacional (16h30-22h30, seg-sex)
...
def imprimir_relatorio_momentum(df: pd.DataFrame, stats_sinais: dict):
    ...
    print(sep)
    print(f"  Velocidade Média (Close diff)        : {v_mean:>12.6f} pontos")
    print(f"  Desvio Padrão Velocidade             : {v_std:>12.6f}")
    print(f"  Velocidade Mínima                    : {v_min:>12.6f} pontos")
    print(f"  Velocidade Máxima                    : {v_max:>12.6f} pontos")
    print(f"  {sub_sep}")
    print(f"  Aceleração Média (Velocidade diff)   : {a_mean:>12.6f} pontos")
    print(f"  Desvio Padrão Aceleração             : {a_std:>12.6f}")
    print(f"  Aceleração Mínima                    : {a_min:>12.6f} pontos")
    print(f"  Aceleração Máxima                    : {a_max:>12.6f} pontos")
    ...
    print(sep)
    print(f"  Stop Loss Médio                       : {sl_mean:>12.2f} pontos")
    print(f"  Take Profit Médio                     : {tp_mean:>12.2f} pontos")
    print(f"  Relação Retorno:Risco (R:R) Confirmada:  1:{rr_ratio:.2f}")
    print(f"{sep}\n")
...
def gerar_grafico_momentum_sinais(df: pd.DataFrame):
    ...
    Painel 1: Preço NASDAQ com setas de compra e venda
    ...
    fig.suptitle("Estratégia Momentum & Entropia — NASDAQ M10", fontsize=16, fontweight='bold', color='#FFFFFF')
    ...
    # offset para o gráfico do nasdaq é maior que no eurusd
    offset = df_plot["Close"].std() * 0.1
    ax1.scatter(compras.index, compras["Close"] - offset, color='#00E676', marker='^', s=45, label='COMPRA (LONG)', zorder=5)
    ax1.scatter(vendas.index, vendas["Close"] + offset, color='#FF1744', marker='v', s=45, label='VENDA (SHORT)', zorder=5)
    
    ax1.set_ylabel("Preço NASDAQ", fontsize=11, color='#CFD8DC')
...
def processar_pipeline_momentum(forcar: bool = False) -> pd.DataFrame:
    ...
    if not PARQUET_ENTRADA.exists():
        raise FileNotFoundError(
            f"Arquivo de entrada de Hurst não encontrado: {PARQUET_ENTRADA}\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Módulo Momentum e Entropia — NASDAQ M10",
    ...
    print("\n" + "█" * 70)
    print("█" + " " * 68 + "█")
    print("█   ESTRATÉGIA DE MOMENTUM E ENTROPIA NASDAQ (M10)                █")
