# -*- coding: utf-8 -*-
"""
Otimização para o GRID 3 — Gatilho baseado em afastamento estatístico por volatilidade.
"""
...
PARAMS_ESPECIFICOS = {
    'janela_zscore':        [50, 100, 150, 200],
    'janela_percentil':     [100, 200, 300],
    'threshold_zscore':     [1.5, 2.0, 2.5, 3.0],
    'threshold_pct_alto':   [0.75, 0.80, 0.85, 0.90],
}

def rodar_otimizacao_grid3():
    """
    Executa a otimização de parâmetros por Grid Search para o Grid 3.
    """
    ...
    logger.info("Carregando base de dados para otimização do Grid 3...")
    ...
    logger.info(f"Iniciando Grid Search do Grid 3: {total_combinacoes} combinações totais.")
    
    # Loop das combinações específicas (para pré-calcular o sinal)
    for i_esp, p_esp in enumerate(combinacoes_especificas):
        if (i_esp + 1) % 20 == 0 or i_esp == 0:
            logger.info(f"Pré-calculando sinal para parâmetros específicos ({i_esp + 1}/{len(combinacoes_especificas)}): {p_esp}")
            
        # O limite inferior é simétrico: 1 - pct_alto
        threshold_pct_baixo = 1.0 - p_esp['threshold_pct_alto']
        
        # Gera o sinal correspondente a essa configuração
        df_sinalizado = gerar_sinal_afastamento(
            df,
            janela_zscore=p_esp['janela_zscore'],
            janela_percentil=p_esp['janela_percentil'],
            threshold_zscore=p_esp['threshold_zscore'],
            threshold_pct_alto=p_esp['threshold_pct_alto'],
            threshold_pct_baixo=threshold_pct_baixo
        )
        
        # Se não houver nenhum sinal disparado, pula a execução dos parâmetros comuns
        if (df_sinalizado['sinal_grid3'] != 0).sum() == 0:
            continue
        ...
        for p_com in combinacoes_comuns:
            ...
            try:
                # Rodar Backtest
                res = rodar_backtest_grid(df_sinalizado, tipo_grid=3, params=params_teste, capital_inicial=CAPITAL_INICIAL, verbose=False)
                
                # Filtro mínimo exigido: total_grids >= 30 e profit_factor > 1.0
                if res['total_grids'] >= 30 and res['profit_factor'] > 1.0:
                    resultados.append({
                        'janela_zscore': params_teste['janela_zscore'],
                        'janela_percentil': params_teste['janela_percentil'],
                        'threshold_zscore': params_teste['threshold_zscore'],
                        'threshold_pct_alto': params_teste['threshold_pct_alto'],
    ...
    caminho_parquet = DIR_OTIM / "otimizacao_grid3_completo.parquet"
    ...
    for idx, row in top10_df.iterrows():
        top10_lista.append({
            "id": f"GRID3_TOP{idx + 1}",
            "tipo_grid": 3,
            "janela_zscore": int(row['janela_zscore']),
            "janela_percentil": int(row['janela_percentil']),
            "threshold_zscore": float(row['threshold_zscore']),
            "threshold_pct_alto": float(row['threshold_pct_alto']),
    ...
    # Salvar JSON Top 10
    caminho_json = DIR_OTIM / "otimizacao_grid3_top10.json"
    ...
    print("\n" + "═"*125)
    print(" " * 48 + "TABELA TOP 10 OTIMIZAÇÃO GRID 3")
    print("═"*125)
    print(f"{'Rank':<5}{'Jan. ZS':<9}{'Jan. Pct':<10}{'Thresh. Z':<10}{'Thresh Pct':<12}{'Mult Esp.':<11}{'Mult Alvo':<11}{'Mult Stop':<11}{'Stop Cand.':<12}{'N Grids':<9}{'FatRecup.':<11}")
    print("─"*125)
    for idx, item in enumerate(top10_lista):
        print(f"#{idx+1:<4}{item['janela_zscore']:<9}{item['janela_percentil']:<10}{item['threshold_zscore']:<10.2f}{item['threshold_pct_alto']:<12.2f}{item['mult_espacamento']:<11.1f}{item['mult_alvo']:<11.1f}{item['mult_stop']:<11.1f}{item['stop_candles']:<12}{item['total_grids']:<9}{item['fator_recuperacao']:<11.2f}")
    print("═"*125 + "\n")

if __name__ == "__main__":
    rodar_otimizacao_grid3()
